...
<template>
  <div ref="pageRoot" class="ffprobe-records plugin-root">
    ...
    <div class="d-flex flex-wrap align-center ga-2 mb-1">
      ...
      <v-alert v-if="notice" type="success" variant="tonal" density="compact" class="status-notice">
        {{ notice }}
      </v-alert>
      <v-spacer />
    ...
    <div class="d-flex flex-wrap align-center ga-2 mb-2">
      ...
      <v-alert v-if="progress.total" type="info" variant="tonal" density="compact" class="inline-status-alert">
        {{ progressText }}
      </v-alert>
      <v-alert v-if="error" type="error" variant="tonal" density="compact" class="inline-status-alert">
        {{ error }}
      </v-alert>
    </div>
    ...
    <div class="records-pagination d-flex flex-wrap align-center ga-2 mt-2">
      <span class="text-body-2">共 {{ total }} 条，每页</span>
      <v-select v-model="pageSize" class="page-size" density="compact" hide-details aria-label="每页数量" :items="pageSizes" @update:model-value="loadPage({ resetPage: true })" />
      ...
      <v-text-field v-model="requestedPage" class="goto-page" density="compact" hide-details aria-label="前往页码" type="number" min="1" :max="pageCount" @keyup.enter="jumpToPage" />
...
<style scoped>
.ffprobe-records { box-sizing: border-box; display: flex; flex-direction: column; position: relative; height: min(calc(100vh - 48px), 62.5rem); min-width: 0; overflow: hidden; padding: 8px 24px 8px; }
.plugin-close-button { position: absolute; top: 8px; right: 8px; z-index: 2; }
.usage-help-button { margin-right: 36px; }
.status-notice { flex: 0 1 auto; }
.inline-status-alert { flex: 1 1 22rem; min-width: 0; }
.inline-status-alert :deep(.v-alert__content) { line-height: 1.5rem; overflow: visible; white-space: normal; }
...
.page-size { width: 72px; }
.goto-page { width: 60px; }
.path-cell { max-width: 520px; overflow-wrap: anywhere; }
@media (max-width: 600px) {
  .ffprobe-records { padding: 16px 16px 8px; }
