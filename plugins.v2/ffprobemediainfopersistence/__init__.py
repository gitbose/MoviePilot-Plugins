...
_FALLBACK_FFPROBE_TIMEOUT_SEC = 10
...
class FFprobeMediaInfoPersistence(_PluginBase):
    ...
    plugin_desc = "优先复用 ffprobe命名补充的缓存，缓存缺失时最多探测 10 秒，并向整理后的媒体目录写入 Emby MediaInfo JSON。"
    plugin_icon = "https://raw.githubusercontent.com/jxxghp/MoviePilot-Plugins/refs/heads/main/icons/ffmpeg.png"
    plugin_version = "1.0.0"
    plugin_author = "DDSRem-Dev community"
    author_url = "https://github.com/DDSRem-Dev/MoviePilot-Plugins"
    ...
    def init_plugin(self, config: dict = None) -> None:
        ...
        if not self._enabled:
            ...
            self._fallback_executor = ThreadPoolExecutor(
                max_workers=type(self)._FALLBACK_WORKERS,
                ...
                "content": [
                    ...
                    },
                    {
                        ...
                        "props": {
                            ...
                            "hint": "关闭后仅复用 ffprobe命名补充的缓存；开启后仅在缓存未命中时，以最多 3 个后台任务对整理目标执行一次 ffprobe。",
                            "persistent-hint": True,
                        },
                    },
                    {
                    ...
                    },
                    {
                        ...
                        "props": {
                            ...
                            "placeholder": "copy\nmove\nlink",
                            "hint": "留空表示不按整理方式过滤。填写后，只有整理事件的方式与其中任一项一致才生成 JSON（不区分大小写）。",
                            "persistent-hint": True,
                            "rows": 3,
                        },
                    },
                    ...
                    },
                    {
                        "component": "VSwitch",
                        "props": {
                            "model": "overwrite_json",
                            "label": "覆盖同名 MediaInfo JSON",
                        },
                    },
                    {
                        "component": "VAlert",
                        "props": {
                            ...
                            "text": (
                                ...
                                "本插件会在整理完成后以最多 3 个后台任务执行一次 ffprobe，"
    ...
    def _background_probe_and_persist(self, destination: Path) -> None:
        ...
        if not destination.is_file():
            ...
            return
        probe = type(self)._run_fallback_ffprobe(str(destination))
    ...
    def _matches_filters(self, destination: Optional[Path], transfer_method: str) -> bool:
        ...
        if self._transfer_methods:
            allowed_methods = {type(self)._normalize(item) for item in self._transfer_methods}
            matches.append(type(self)._normalize(transfer_method) in allowed_methods)
    ...
    @classmethod
    def _run_fallback_ffprobe(cls, media_path: str) -> Optional[Dict[str, Any]]:
        """缓存缺失时的唯一兜底：只执行一次 ffprobe，超过 10 秒立即放弃。"""
        ...
        try:
            process = run(
                ...
                capture_output=True,
                text=True,
                timeout=_FALLBACK_FFPROBE_TIMEOUT_SEC,
            )
        except TimeoutExpired:
            logger.warning("【ffprobe媒体信息持久化】兜底 ffprobe 超时（10 秒），跳过：%s", probe_target)
            return None
        except OSError as error:
